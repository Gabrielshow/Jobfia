	...
	$results_per_page = 10;
		...
		if (isset($_POST['action']) && $_POST['action'] == "delete")
		{
			...
			if (count($ids_arr) > 0)
			{
				...
				header("Location: job_reports.php?msg=deleted");
				exit();
			}
		}
		...
		$query = "SELECT reports.*, DATE_FORMAT(reports.added, '%d %b %Y %h:%i %p') AS date_added, jobs.* FROM jobfia_reports reports LEFT JOIN jobfia_jobs jobs ON jobs.job_id=reports.job_id WHERE reports.job_id<>'0' ORDER BY reports.added DESC LIMIT $from, $results_per_page";

		$result = smart_mysql_query($query);
		$total_on_page = mysqli_num_rows($result);

		$query2 = "SELECT * FROM jobfia_reports WHERE job_id<>'0'";
...
?>
		...
		<h2><i class="fa fa-flag" style="color: #c9302c"></i> Jobs Reports</h2>
        ...
        <?php if ($total > 0) { ?>
			...
			<?php while ($row = mysqli_fetch_array($result)) { $cc++; ?>				  
				  <tr class="<?php if (($cc%2) == 0) echo "even"; else echo "odd"; ?>">
					...
					<td nowrap="nowrap" align="center" valign="middle"><a href="job_details.php?id=<?php echo $row['job_id']; ?>&pn=<?php echo $page; ?>&column=<?php echo $_GET['column']; ?>&order=<?php echo $_GET['order']; ?>"><?php echo $row['job_id']; ?></a></td>
					...
					<td nowrap="nowrap" align="center" valign="middle">
						...
						<a href="#" onclick="if (confirm('Are you sure you really want to delete this report?') )location.href='report_delete.php?id=<?php echo $row['report_id']; ?>&column=<?php echo $_GET['column']; ?>&order=<?php echo $_GET['order']; ?>&pn=<?php echo $page?>&type=jobs'" title="Delete"><img src="images/delete.png" border="0" alt="Delete" /></a>
					</td>
				  </tr>
			<?php } ?>
				<tr>
				...
				<?php echo ShowPagination("reports",$results_per_page,"job_reports.php?column=$rrorder&order=$rorder&","WHERE job_id<>'0'"); ?>

          <?php }else{ ?>
					<div class="alert alert-info">There are no reports at this time.</div>
          <?php } ?>
