...
\*******************************************************************/
	...
	if (isset($_GET['id']) && is_numeric($_GET['id']))
	{
		...
		$jquery = "SELECT *, DATE_FORMAT(added, '%d %b %Y <sup>%h:%i %p</sup>') AS date_added FROM jobfia_jobs WHERE job_id='$job_id' LIMIT 1";
...
?>
     ...
     <h2><i class="fa fa-check-circle-o"></i> Job Applicants <?php if ($total > 0) { ?> - <a href="job_details.php?id=<?php echo $jrow['job_id']; ?>"><?php echo $jrow['title']; ?></a><?php } ?> <?php if ($total > 0) echo "<sup class='badge'>".$total."</sup>"; ?> <?php if (@$filter) { ?> / Search results<?php } ?></h2>
		...
		<tr height="20">
			<td width="33%" bgcolor="#F4C869" align="center"><span style="font-size: 18px; color: #FFF;" class="jobfia_tooltip" title="awaiting review"><?php echo GetJobPendingTasks($job_id); ?></span></td>
			<td width="33%" bgcolor="#92D811" align="center"><span style="font-size: 18px; color: #FFF;" class="jobfia_tooltip" title="satisfied"><?php echo GetJobPaidTasks($job_id); ?></span></td>
			<td width="33%" bgcolor="#F9AA98" align="center"><span style="font-size: 18px; color: #FFF;" class="jobfia_tooltip" title="not satisfied"><?php echo GetJobDeclinedTasks($job_id); ?></span></td>
		</tr>
		</table>
		</div>
		<?php } ?>
		...
		<tr>
			...
			<td nowrap="nowrap" width="30%" valign="middle" align="left">
				<div class="admin_filter">
					...
					<?php if (isset($filter) && $filter != "") { ?><a title="Cancel Search" href="job_tasks.php?id=<?php echo $job_id; ?>"><img align="absmiddle" src="images/icons/delete_filter.png" border="0" alt="Cancel Search" /></a><?php } ?> 
				</div>
			</td>
			...
			<table align="center" width="100%" border="0" cellspacing="0" cellpadding="3">
              <tr>
				...
				<th width="25%">Proof submitted</th>
			...
			<?php while ($row = mysqli_fetch_array($result)) { $cc++; ?>
                <tr class="<?php if (($cc%2) == 0) echo "even"; else echo "odd"; ?>">
                  ...
                  <td valign="middle" align="left"><?php echo GetUsername($row['worker_id']); ?></td>
                  <td valign="middle" align="left">
					<?php
							if (strlen($row['job_proof']) > 160)
							{
								$job_proof = substr($row["job_proof"], 0, 155);
                  ...
                  <td nowrap="nowrap" valign="middle" align="center">
					<?php if ($row['proof_file'] != "") { ?><a href="<?php echo substr(SITE_URL, 0, -1).JOBFIA_PROOFS_URL.$row['proof_file']; ?>" class="jobfia_tooltip" title="<?php echo $row['proof_downloads']; ?> downloads"><i class="fa fa-paperclip fa-lg"></i></a><?php }else{ ?>---<?php } ?>	                  
                  </td>
			...
			<?php } ?>
				...
				<tr>
					<td colspan="7" align="center">
						 <?php echo ShowPagination("job_tasks",$results_per_page,"job_tasks.php?id=$job_id&column=$rrorder&order=$rorder&show=$results_per_page&","WHERE job_id='$job_id'".$filter_by); ?>
					</td>
				</tr>

	<?php }else{ ?>
			<?php if (isset($filter)) { ?>
				<tr><td colspan="7"><div class="alert alert-info text-center">No applicants found. <a class="btn btn-success" href="job_tasks.php?id=<?php echo $job_id; ?>">Search again &#155;</a></div></td></tr>
			<?php }else{ ?>
				<tr><td colspan="7"><div class="alert alert-info text-center">No submitted applicants at this time.</div></td></tr>
			<?php } ?>
	<?php } ?>
